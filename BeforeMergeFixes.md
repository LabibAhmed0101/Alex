...
- **A rotated lead arrives as a Cold Lead**, as specification describes. Before this, a lead that rotated while at Shopped Out landed on the new broker still reading Shopped Out — a stage they had nothing to do with reaching.
...
- **A screenshot in the code repository contained a real merchant's Social Security number**, along with their EIN, date of birth and thirteen phone numbers. It has been removed and the folder it sat in is now blocked so nothing like it can be added again.
...
- **Emailed attachments were being filed as bank statements** whether they were or not. They now arrive tagged "Other" for you to re-tag.
### Things that were simply broken

- **The email reading feature had never worked.** It was pointed at an AI service that does not exist, so every attempt failed silently. That is now fixed — though the feature stays switched off for now.
